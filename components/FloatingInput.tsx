...
export default function FloatingInput({ placeholder, name, type, required = false }: FloatingInputProps) {
    return (
        <div className="relative">
            ...
            <label
                htmlFor={name}
                className="mt-1 bg-white dark:bg-slate-900 px-1 absolute left-5 top-5 text-gray-500 dark:text-white transition-position duration-200 peer-placeholder-shown:top-2 peer-placeholder-shown:text-sm peer-focus:top-[-13px] peer-focus:text-xs peer-focus:text-primary-400 peer-[&:not(:placeholder-shown)]:top-[-13px] peer-[&:not(:placeholder-shown)]:text-xs"
            >
                {placeholder}
            </label>
